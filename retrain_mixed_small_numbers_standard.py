#!/usr/bin/env python3
"""
Retrain Mixed Small Numbers model with standardized evaluation.
Based on run s8snd2a0 which achieved 60.7% (but on easier 0-10 test set).
...
def create_mixed_small_numbers_dataset(n_samples=150):
    """Create mixed dataset with small numbers (0-10)."""
    prompts = []
    answers = []
    task_types = []
    ...
    for _ in range(n_counting):
        ...
        words = random.sample(['cat', 'dog', 'bird', 'fish', 'tree', 'car', 
                             ...
                             'computer', 'window', 'door', 'light'], num_words)
        sentence = ' '.join(words)
...
def main():
    ...
    # Training configuration - matching successful s8snd2a0
    n_samples = 150
    batch_size = 240  
    num_generations = 16
    learning_rate = 5e-6
    temperature = 0.7
    epochs = 100  # Extended training like the successful run
    
    # Initialize WandB
    wandb.init(
        project=os.getenv("WANDB_PROJECT", "pippa"),
        entity=os.getenv("WANDB_ENTITY", "wild-ai"),
        name="retrain_mixed_small_numbers_standard",
        tags=["grpo-setup", "standardized-eval", "mixed-dataset", "small-numbers", "retrain"],
        config={
            "model": model_name,
            "n_samples": n_samples,
            ...
            "task_distribution": "50% arithmetic (0-10), 25% counting, 25% comparison",
            "based_on_run": "s8snd2a0"
        }
    )
    
    print("Creating mixed small numbers dataset...")
    dataset = create_mixed_small_numbers_dataset(n_samples)
    ...
    # GRPO configuration
    config = GRPOConfig(
        output_dir="./grpo-mixed-small-standard",
        run_name="retrain_mixed_small_numbers_standard",

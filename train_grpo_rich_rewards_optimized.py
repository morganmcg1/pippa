#!/usr/bin/env python3
"""
GRPO training with optimized rich reward schedule.
Testing different reward curves for better learning.
"""
...
def optimized_reward_function(samples: List[str], prompts: List[str], answers: List[str], **kwargs) -> List[float]:
    """
    Optimized reward function with smoother decay.
    Using exponential-like decay instead of stepped rewards.
    """
    rewards = []
    ...
    for sample, prompt, expected in zip(samples, prompts, answers):
        ...
        try:
            ...
            # Exponential-like decay: reward = 1.0 * decay^distance
            # But capped at minimum -0.5
            if distance == 0:
                reward = 1.0
            else:
                # Decay factor of 0.7 per unit distance
                reward = max(0.7 ** distance, -0.5)
                # Slight bonus for being very close
                if distance == 1:
...
def main():
    ...
    wandb.init(
        ...
        name="grpo_rich_rewards_optimized",
        tags=["grpo-setup", "standardized-eval", "rich-rewards", "optimized-schedule"],
        config={
            "model": model_name,
            "n_samples": n_samples,
    ...
    def reward_wrapper(completions, prompts=None, **kwargs):
        ...
        return optimized_reward_function(completions, prompts, answers, **kwargs)
    
    # GRPO configuration
    config = GRPOConfig(
        output_dir="./grpo-rich-optimized",
        run_name="grpo_rich_rewards_optimized",
    ...
    print("\n" + "="*60)
    print("OPTIMIZED RICH REWARDS EXPERIMENT")
    print("="*60)
    print("Reward schedule (exponential decay):")
    print("- Correct: 1.0")
    print("- Off by 1: 0.8")
    print("- Off by 2: 0.5")
    print("- Off by 3: 0.34")
    print("- Off by 4: 0.24")
    print("- Off by 5+: exponential decay (0.7^distance)")
    print("- Non-numeric: -1.0")
    print("\nSmoother gradient for better learning!")
    print("="*60 + "\n")
    ...
    print("Starting training with optimized rich rewards...")
    trainer.train()
    ...
    print(f"\n" + "="*60)
    print("FINAL RESULTS - Optimized Rich Rewards")
    print("="*60)

#!/usr/bin/env python3
"""
GRPO training with rich rewards + larger dataset.
Testing if more training data improves on 75% accuracy.
"""
...
def create_larger_dataset(n_samples=500):
    """Create larger arithmetic dataset with 0-10 numbers."""
    prompts = []
    answers = []
    
    operations = ['+', '-', '*']
    
    # Ensure good coverage of all combinations
    samples_generated = 0
    attempts = 0
    seen_problems = set()
    
    while samples_generated < n_samples and attempts < n_samples * 3:
    ...
    print(f"Generated {len(prompts)} unique arithmetic problems")
...
def main():
    ...
    n_samples = 500  # 5x more data
    batch_size = 512  # Larger batch to utilize data
    num_generations = 16
    learning_rate = 5e-6
    temperature = 0.7
    epochs = 50
    ...
    wandb.init(
        ...
        name="grpo_rich_500_samples",
        tags=["grpo-setup", "standardized-eval", "rich-rewards", "more-data"],
        config={
            "model": model_name,
            "n_samples": n_samples,
    ...
    print(f"Creating larger dataset ({n_samples} samples, 0-10)...")
    dataset = create_larger_dataset(n_samples)
    ...
    # GRPO configuration
    config = GRPOConfig(
        output_dir="./grpo-rich-500-samples",
        run_name="grpo_rich_500_samples",
    ...
    print("\n" + "="*60)
    print("RICH REWARDS + MORE DATA EXPERIMENT")
    print("="*60)
    print(f"Training with {n_samples} unique arithmetic problems")
    print("5x more data than the 75% baseline (130 samples)")
    print("Hypothesis: More diverse training data → better generalization")
    print("Previous best: 75% with 130 samples")
    print("="*60 + "\n")
    ...
    print(f"Previous rich rewards best (130 samples): 75.0%")
    print("="*60)

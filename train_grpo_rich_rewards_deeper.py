#!/usr/bin/env python3
"""
GRPO training with rich rewards + deeper reward schedule.
Testing finer-grained rewards for distances 1-5.
"""
...
def deeper_reward_function(samples: List[str], prompts: List[str], answers: List[str], **kwargs) -> List[float]:
    """
    Deeper reward function with finer-grained partial credit.
    More nuanced rewards for small distances.
    """
    rewards = []
    ...
    for sample, prompt, expected in zip(samples, prompts, answers):
        ...
        try:
            ...
            # Finer-grained rewards for distances 1-5
            if distance == 0:
                reward = 1.0
            elif distance == 1:
                reward = 0.8  # Higher reward for being very close
            elif distance == 2:
                reward = 0.6
            elif distance == 3:
                reward = 0.45
...
def main():
    ...
    wandb.init(
        ...
        name="grpo_rich_deeper_rewards",
        tags=["grpo-setup", "standardized-eval", "rich-rewards", "deeper-schedule"],
        config={
            "model": model_name,
            "n_samples": n_samples,
            ...
            "number_range": "0-10",
            "reward_schedule": "deeper_granular"
        }
    )
    ...
    def reward_wrapper(completions, prompts=None, **kwargs):
        ...
        return deeper_reward_function(completions, prompts, answers, **kwargs)
    
    # GRPO configuration
    config = GRPOConfig(
        output_dir="./grpo-rich-deeper",
        run_name="grpo_rich_deeper_rewards",
    ...
    print("\n" + "="*60)
    print("DEEPER RICH REWARDS EXPERIMENT")
    print("="*60)
    print("Reward schedule (finer-grained):")
    print("- Correct: 1.0")
    print("- Off by 1: 0.8 (was 0.7)")
    print("- Off by 2: 0.6 (was 0.4)")
    print("- Off by 3: 0.45 (new)")
    print("- Off by 4: 0.3 (new)")
    print("- Off by 5: 0.15 (new)")
    print("- Off by 6-7: 0.0 (new)")
    print("- Off by 8-10: -0.2")
    print("- Off by >10: -0.5")
    print("- Non-numeric: -1.0")
    print("\nMore granular feedback for near-misses!")
    print("="*60 + "\n")
    ...
    print("Starting training with deeper rich rewards...")
    trainer.train()
    ...
    print(f"\n" + "="*60)
    print("FINAL RESULTS - Deeper Rich Rewards")
    print("="*60)

#!/usr/bin/env python3
"""
Train PPO on Gymnasium-Robotics Fetch environments with proper WandB integration.
This version ensures all metrics and videos are properly logged to WandB.
"""
...
def make_fetch_env(env_id: str, idx: int, capture_video: bool, run_name: str, 
    ...
    def thunk():
        ...
        if capture_video and idx == 0:
            # Record every 10 episodes for better visibility
...
def train(args):
    ...
    episode_count = 0
    recent_episodes = deque(maxlen=100)  # Track last 100 episodes
    ...
    for update in range(1, num_updates + 1):
        ...
        if args.track and WANDB_AVAILABLE and args.capture_video:
            ...
            if video_dir.exists() and (update - last_video_check) >= 5:  # Check every 5 updates
                last_video_check = update
                video_files = sorted(video_dir.glob("*.mp4"))
                
                for video_file in video_files:
                    if str(video_file) not in uploaded_videos:
                        try:
                            # Upload video with proper metadata
                            wandb.log({
                                "video": wandb.Video(str(video_file), fps=30, format="mp4"),
                                "video_episode": int(video_file.stem.split('-')[-1]) if '-' in video_file.stem else 0,
                                "global_step": global_step,
                            })
                            uploaded_videos.add(str(video_file))
    ...
    if args.track and WANDB_AVAILABLE:
        ...
        # Upload any remaining videos
        if args.capture_video:
            video_dir = Path(f"videos/{run_name}")
            if video_dir.exists():
                remaining_videos = sorted(video_dir.glob("*.mp4"))
                for video_file in remaining_videos:
                    if str(video_file) not in uploaded_videos:
                        try:
                            wandb.log({
                                "final_video": wandb.Video(str(video_file), fps=30, format="mp4"),
                            })
                            print(f"Uploaded final video: {video_file.name}")
                        except Exception as e:
                            print(f"Failed to upload final video {video_file}: {e}")

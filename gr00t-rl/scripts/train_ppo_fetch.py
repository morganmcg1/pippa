...
def train(args):
    ...
    for update in range(1, num_updates + 1):
        ...
        # Debug: Check what type rollout_data is
        print(f"DEBUG: rollout_data type = {type(rollout_data)}")
        if hasattr(rollout_data, '__iter__') and not isinstance(rollout_data, dict):
            print("DEBUG: rollout_data is iterable but not a dict, converting...")
            # If it's a generator, get the first (and only) batch
            rollout_data = next(rollout_data)

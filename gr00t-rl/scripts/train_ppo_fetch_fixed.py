...
Fixed version with proper WandB logging and video capture.
"""
...
def make_fetch_env(env_id: str, idx: int, capture_video: bool, run_name: str, 
    ...
    def thunk():
        ...
        if capture_video and idx == 0:
            # Use a more frequent video recording schedule for debugging
            env = gym.wrappers.RecordVideo(
                env, f"videos/{run_name}",
                episode_trigger=lambda episode_id: episode_id % 10 == 0,  # Record every 10 episodes
                disable_logger=True
            )
...
def train(args):
    ...
    if args.track and WANDB_AVAILABLE:
        wandb.init(
            ...
            mode="online"  # Ensure online mode for real-time updates
        )
        # Define custom x-axis for proper metric display
    ...
    for update in range(1, num_updates + 1):
        ...
        # Metrics to accumulate for this update
        update_episode_rewards = []
        update_episode_lengths = []
        update_successes = []
        update_final_distances = []
        ...
        for step in range(0, args.num_steps):
            ...
            for idx, info in enumerate(infos):
                if "episode" in info:
                    ...
                    update_episode_rewards.append(episode_info["return"])
                    update_episode_lengths.append(episode_info["length"])
                    update_successes.append(episode_info["success"])
                    update_final_distances.append(episode_info["final_distance"])
        ...
        # Prepare all metrics for single WandB log call
        metrics = {
        ...
        # Add episode metrics if any episodes finished this update
        if update_episode_rewards:
            metrics.update({
                "episode/reward_mean": np.mean(update_episode_rewards),
                "episode/reward_std": np.std(update_episode_rewards),
                "episode/length_mean": np.mean(update_episode_lengths),
                "episode/success_rate": np.mean(update_successes),
                "episode/final_distance_mean": np.mean(update_final_distances),
            })
        ...
        # Single WandB log call per update
        if args.track and WANDB_AVAILABLE:
            wandb.log(metrics)
            ...
            if args.capture_video and update % 10 == 0:
                ...
                if video_dir.exists():
                    ...
                    for vf in video_files[-3:]:  # Log last 3 videos
                        try:
                            wandb.log({
                                f"videos/{vf.stem}": wandb.Video(str(vf), fps=30, format="mp4"),
                                "global_step": global_step
                            })
                        except Exception as e:
                            print(f"Failed to log video {vf}: {e}")
        ...
        if update % 10 == 0:
            ...
            if update_episode_rewards:
                print(f"  Episode Reward Mean: {np.mean(update_episode_rewards):.3f}")
                print(f"  Episodes Completed: {len(update_episode_rewards)}")

#!/usr/bin/env python3
"""
Train ensemble of 3 GRPO models with rich rewards using different seeds.
Average predictions for more robust performance.
"""
...
def evaluate_ensemble(models, tokenizers, device) -> Dict[str, float]:
    """Evaluate ensemble of models on the standardized dataset."""
    ...
    for i, sample in enumerate(eval_dataset):
        ...
        for model, tokenizer in zip(models, tokenizers):
            ...
            with torch.no_grad():
                ...
                try:
                    ...
                    pass  # Skip non-numeric predictions
        ...
        if all_predictions:
            ...
            if vote_counts:
                ensemble_pred = str(vote_counts.most_common(1)[0][0])
            else:
                ensemble_pred = ""
        else:
            ensemble_pred = ""
        
        is_correct = ensemble_pred == expected
        if is_correct:
            correct_total += 1
...
def evaluate_on_standard_dataset(model, tokenizer, device) -> Dict[str, float]:
    """Evaluate single model on the standardized arithmetic evaluation dataset."""
    eval_dataset = load_dataset("morgan/arithmetic_eval", split="test")
...
def train_single_model(seed_idx, seed):
    """Train a single model with specific seed."""
    ...
    learning_rate = 5e-6
    temperature = 0.7
    epochs = 30  # Reduced for ensemble
    
    # Initialize WandB
    wandb.init(
        project=os.getenv("WANDB_PROJECT", "pippa"),
        entity=os.getenv("WANDB_ENTITY", "wild-ai"),
        name=f"grpo_rich_ensemble_model{seed_idx+1}",
        tags=["grpo-setup", "standardized-eval", "rich-rewards", "ensemble", f"model{seed_idx+1}"],
        config={
            "model": model_name,
            "n_samples": n_samples,
            ...
            "ensemble_model": seed_idx + 1
        }
    )
    
    print(f"\n{'='*60}")
    print(f"Training Ensemble Model {seed_idx + 1}/3 (seed={seed})")
    print(f"{'='*60}\n")
    ...
    # GRPO configuration
    config = GRPOConfig(
        output_dir=f"./grpo-ensemble-model{seed_idx+1}",
        run_name=f"grpo_rich_ensemble_model{seed_idx+1}",
    ...
    print(f"Starting training for model {seed_idx + 1}...")
    trainer.train()
    
    print(f"\nModel {seed_idx + 1} training completed!")
    ...
    print(f"\nEvaluating model {seed_idx + 1}...")
    eval_metrics = evaluate_on_standard_dataset(model, tokenizer, device)
    
    wandb.log({
        f"model{seed_idx+1}/eval_accuracy": eval_metrics['eval_accuracy'],
        **{f"model{seed_idx+1}/{k}": v for k, v in eval_metrics.items()}
    })
    
    print(f"Model {seed_idx + 1} accuracy: {eval_metrics['eval_accuracy']:.2%}")
    
    wandb.finish()
    
    return model, tokenizer, eval_metrics['eval_accuracy']

def main():
    print("\n" + "="*60)
    print("ENSEMBLE RICH REWARDS EXPERIMENT")
    print("="*60)
    print("Training 3 models with different seeds")
    print("Will use majority voting for final predictions")
    print("="*60 + "\n")
    ...
    for i, seed in enumerate(SEEDS):
        model, tokenizer, accuracy = train_single_model(i, seed)
        models.append(model)
        tokenizers.append(tokenizer)
        individual_accuracies.append(accuracy)
    
    # Initialize WandB for ensemble evaluation
    wandb.init(
        project=os.getenv("WANDB_PROJECT", "pippa"),
        entity=os.getenv("WANDB_ENTITY", "wild-ai"),
        name="grpo_rich_ensemble_final",
        tags=["grpo-setup", "standardized-eval", "rich-rewards", "ensemble", "final"],
        config={
            "ensemble_size": 3,
    ...
    device = models[0].device if hasattr(models[0], 'device') else next(models[0].parameters()).device
    
    print("Running ensemble evaluation...")
    ensemble_metrics = evaluate_ensemble(models, tokenizers, device)
    
    wandb.log({
        "ensemble/eval_accuracy": ensemble_metrics['eval_accuracy'],
        **{f"ensemble/{k}": v for k, v in ensemble_metrics.items()}
    })
    
    print(f"\n" + "="*60)
    print("FINAL RESULTS - Ensemble")
    print("="*60)
    print(f"Individual model accuracies: {[f'{acc:.2%}' for acc in individual_accuracies]}")
    print(f"Ensemble accuracy: {ensemble_metrics['eval_accuracy']:.2%}")
    print(f"Previous single model best: 75.0%")
    print("="*60)

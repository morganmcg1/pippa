#!/usr/bin/env python3
"""
Retrain Mixed Dataset Full Diversity model with standardized evaluation.
Based on run afj0flv3 which achieved 54.7% (on 0-20 test set).
...
def main():
    ...
    # Training configuration - matching successful afj0flv3
    n_samples = 150
    batch_size = 240  
    num_generations = 16  # Full diversity
    learning_rate = 5e-6
    temperature = 0.7
    epochs = 75  # Extended from original 50
    
    # Initialize WandB
    wandb.init(
        project=os.getenv("WANDB_PROJECT", "pippa"),
        entity=os.getenv("WANDB_ENTITY", "wild-ai"),
        name="retrain_mixed_dataset_standard",
        tags=["grpo-setup", "standardized-eval", "mixed-dataset", "full-diversity", "retrain"],
        config={
            "model": model_name,
            "n_samples": n_samples,
    ...
    # GRPO configuration
    config = GRPOConfig(
        output_dir="./grpo-mixed-standard",
        run_name="retrain_mixed_dataset_standard",

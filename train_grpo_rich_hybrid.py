#!/usr/bin/env python3
"""
GRPO training with hybrid approach: rich rewards + mixed tasks (arithmetic only).
Mix different arithmetic difficulties in one dataset.
"""
...
def create_hybrid_dataset(n_samples=256):
    """Create mixed difficulty arithmetic dataset."""
    prompts = []
    answers = []
    metadata = []
    ...
    # Define difficulty distributions (50% easy, 30% medium, 20% hard)
    difficulty_ranges = {
        'easy': (0, 5),      # 50% of samples
        'medium': (0, 10),   # 30% of samples  
        'hard': (5, 15),     # 20% of samples
    }
    
    difficulty_distribution = [
        ('easy', int(n_samples * 0.5)),
        ('medium', int(n_samples * 0.3)),
        ('hard', n_samples - int(n_samples * 0.5) - int(n_samples * 0.3))  # Remainder
    ]
    
    for difficulty, count in difficulty_distribution:
        min_num, max_num = difficulty_ranges[difficulty]
    ...
    combined = list(zip(prompts, answers, metadata))
    random.shuffle(combined)
    prompts, answers, metadata = zip(*combined)
    
    print(f"\nCreated hybrid dataset:")
    print(f"- Easy (0-5): {difficulty_distribution[0][1]} samples")
    print(f"- Medium (0-10): {difficulty_distribution[1][1]} samples")
    print(f"- Hard (5-15): {difficulty_distribution[2][1]} samples")
...
def main():
    ...
    wandb.init(
        ...
        name="grpo_rich_hybrid",
        tags=["grpo-setup", "standardized-eval", "rich-rewards", "hybrid-difficulty"],
        config={
            "model": model_name,
            "n_samples": n_samples,
    ...
    print("Creating hybrid difficulty dataset...")
    dataset = create_hybrid_dataset(n_samples)
    ...
    # GRPO configuration
    config = GRPOConfig(
        output_dir="./grpo-rich-hybrid",
        run_name="grpo_rich_hybrid",
    ...
    print("\n" + "="*60)
    print("HYBRID DIFFICULTY EXPERIMENT")
    print("="*60)
    print("Mixed difficulty arithmetic (all with rich rewards):")
    print("- 50% Easy problems (0-5)")
    print("- 30% Medium problems (0-10)")
    print("- 20% Hard problems (5-15)")
    print("\nHypothesis: Natural curriculum through mixed difficulties")
    print("Previous best: 75% with uniform 0-10 range")
    print("="*60 + "\n")
    ...
    print("Starting training with hybrid difficulty dataset...")
    trainer.train()
    ...
    print(f"\n" + "="*60)
    print("FINAL RESULTS - Hybrid Difficulty")
    print("="*60)

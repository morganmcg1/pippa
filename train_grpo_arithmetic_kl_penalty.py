#!/usr/bin/env python3
"""GRPO arithmetic overfitting with KL penalty for stability."""
...
def main():
    # Configuration matching successful run ljokc85g
    n_samples = 100  # Same as successful run
    batch_size = 64  # Same as successful run
    num_generations = 16  # Same as successful run
    lr = 5e-6  # Same as successful run
    temperature = 0.7  # Same as successful run
    epochs = 20  # Same as successful run
    seed = 42  # Same as successful run
    beta = 0.1  # KL penalty - IMPORTANT!
    ...
    run = wandb.init(
        ...
        name=f"grpo_arithmetic_kl_penalty_b{batch_size}_g{num_generations}",
        config={
            "task": "arithmetic_with_kl",
            "n_samples": n_samples,
        ...
        tags=["grpo-setup", "overfit", "arithmetic", "kl-penalty"]
    )
    ...
    # Print dataset info
    print(f"\n{'='*60}")
    print(f"ARITHMETIC GRPO WITH KL PENALTY")
    print(f"{'='*60}")
    ...
    # GRPO config - matching successful run
    config = GRPOConfig(
        output_dir=f"./grpo_arithmetic_kl_{seed}",
        ...
        # Optimization parameters from successful run
        warmup_ratio=0.1,
        weight_decay=0.01,
        max_grad_norm=1.0,
        optim="adamw_torch",
        scale_rewards=True,  # Important for standard GRPO
    )

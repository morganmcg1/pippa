#!/usr/bin/env python3
"""
Retrain with extended epochs (100) using standardized evaluation.
Based on the arithmetic baseline configuration with long training.
...
def create_arithmetic_dataset(n_samples=100):
    """Create simple arithmetic dataset (0-20)."""
    prompts = []
    answers = []
...
def main():
    ...
    # Training configuration - based on arithmetic baseline with extended epochs
    n_samples = 100
    batch_size = 64  
    num_generations = 16
    learning_rate = 5e-6
    ...
    wandb.init(
        ...
        name="retrain_long_epochs_standard",
        tags=["grpo-setup", "standardized-eval", "arithmetic", "long-epochs", "retrain"],
        config={
            "model": model_name,
            "n_samples": n_samples,
            ...
            "number_range": "0-20",
            "based_on_config": "arithmetic_baseline_long_epochs"
        }
    )
    
    print("Creating arithmetic dataset...")
    dataset = create_arithmetic_dataset(n_samples)
    ...
    # GRPO configuration
    config = GRPOConfig(
        output_dir="./grpo-long-epochs-standard",
        run_name="retrain_long_epochs_standard",
        ...
        save_strategy="steps",
        save_steps=500,
        save_total_limit=3,  # Keep more checkpoints for long training
        report_to="wandb",
    ...
    print(f"Improvement over baseline: {standard_eval_metrics['standardized_eval_accuracy'] - 0.38:.2%}")
    print("="*60)

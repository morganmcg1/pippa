#!/usr/bin/env python3
"""
GRPO training with rich rewards + longer training (100 epochs).
Testing if we can push past 75% with more training.
"""
...
# Custom GRPOTrainer to fix log_completions issue and add periodic eval
class GRPOTrainerFixed(GRPOTrainer):
    ...
    def log(self, logs: Dict[str, Any], start_time: float = None) -> None:
        ...
        if hasattr(self.state, 'epoch') and self.state.epoch:
            if int(self.state.epoch) % 10 == 0 and int(self.state.epoch) > 0:
                if not hasattr(self, '_last_eval_epoch') or self._last_eval_epoch != int(self.state.epoch):
                    ...
                    wandb.log({
                        f"epoch{int(self.state.epoch)}/eval_accuracy": eval_metrics['eval_accuracy'],
                        **{f"epoch{int(self.state.epoch)}/{k}": v for k, v in eval_metrics.items()}
                    }, step=self.state.global_step)
                    
                    print(f"Epoch {int(self.state.epoch)} evaluation accuracy: {eval_metrics['eval_accuracy']:.2%}")
...
def main():
    ...
    wandb.init(
        ...
        name="grpo_rich_100_epochs",
        tags=["grpo-setup", "standardized-eval", "rich-rewards", "long-training"],
        config={
            "model": model_name,
            "n_samples": n_samples,
            ...
            "number_range": "0-10",
            "eval_frequency": "every_10_epochs"
        }
    )
    ...
    # GRPO configuration
    config = GRPOConfig(
        output_dir="./grpo-rich-100epochs",
        run_name="grpo_rich_100_epochs",
    ...
    print("\n" + "="*60)
    print("RICH REWARDS + 100 EPOCHS EXPERIMENT")
    print("="*60)
    print("Testing extended training (100 epochs)")
    print("Evaluation every 10 epochs to track progress")
    print("Hypothesis: More training might push past 75%")
    print("Previous best: 75% at 50 epochs")
    print("="*60 + "\n")
    ...
    print("Starting training with rich rewards for 100 epochs...")
    trainer.train()
    ...
    print(f"\n" + "="*60)
    print("FINAL RESULTS - Rich Rewards + 100 Epochs")
    print("="*60)
    print(f"Final standardized eval accuracy: {final_eval_metrics['eval_accuracy']:.2%}")
    print(f"Previous rich rewards best (50 epochs): 75.0%")
    print("="*60)

#!/usr/bin/env python3
"""
GRPO Arithmetic Training - Smaller Numbers with FULL Diversity
Uses numbers 0-10 AND maintains proper generation diversity
Uses batch_size=256 to support 16 generations per prompt
"""
...
def main():
    ...
    # Training configuration - FULL DIVERSITY WITH SMALLER NUMBERS
    ...
    num_generations = 16  # RESTORED to baseline
    learning_rate = 5e-6
    temperature = 0.7
    epochs = 50
    ...
    wandb.init(
        ...
        name="grpo_arithmetic_smaller_numbers_full_diversity",
        tags=["grpo-setup", "arithmetic", "overfit", "break-barrier", "small-numbers", "full-diversity"],
        config={
            "model": model_name,
            "n_samples": n_samples,
            "max_number": max_num,
            ...
            "seed": SEED,
            "beta": 0.1,
            "experiment": "smaller_numbers_full_diversity",
    ...
    print(f"\n{'='*60}")
    print("SMALLER NUMBERS WITH FULL DIVERSITY")
    print(f"{'='*60}")
    print(f"Number range: 0-{max_num} (easier than 0-20)")
    ...
    config = GRPOConfig(
        output_dir="./grpo-arithmetic-small-nums-full-div",
    ...
    print("\nStarting training with smaller numbers and FULL diversity...")
    print(f"Hypothesis: Numbers 0-{max_num} + proper diversity = breakthrough performance")
    trainer.train()

#!/usr/bin/env python3
"""
GRPO Mixed Dataset + Small Numbers (0-10) Experiment
Combines the 54.7% mixed dataset approach with easier 0-10 number range
Target: >60% final accuracy by combining best strategies
"""
...
def create_mixed_small_dataset(n_samples=150):
    """Create a mixed dataset with small numbers (0-10) only"""
    prompts = []
    answers = []
    task_types = []
    ...
    # Arithmetic tasks (0-10 only)
    operations = ['+', '-', '*']
    for _ in range(n_arithmetic):
        a = random.randint(0, 10)  # Reduced from 0-20
        b = random.randint(0, 10)  # Reduced from 0-20
    ...
    # Counting tasks (0-10 only)
    for _ in range(n_counting):
        count = random.randint(1, 10)  # Already 1-10
...
def main():
    ...
    # Training configuration - Same as 54.7% success but 100 epochs
    n_samples = 150
    batch_size = 240  # Same as successful run
    num_generations = 16  # Same as successful run
    learning_rate = 5e-6
    temperature = 0.7
    ...
    wandb.init(
        ...
        name="grpo_mixed_small_numbers",
        tags=["grpo-setup", "arithmetic", "mixed-tasks", "small-numbers", "break-60"],
        config={
            "model": model_name,
            "n_samples": n_samples,
            ...
            "seed": SEED,
            "beta": 0.1,
            "experiment": "mixed_small_numbers",
            "task_distribution": "50% arithmetic, 25% counting, 25% comparison",
            "number_range": "0-10 only",
            "note": "Combines 54.7% mixed approach with easier small numbers"
        }
    )
    
    print("Preparing training configuration...")
    print(f"\n{'='*60}")
    print("MIXED DATASET + SMALL NUMBERS (0-10)")
    print(f"{'='*60}")
    print(f"Combining best strategies:")
    print(f"  - Mixed tasks (from 54.7% success)")
    print(f"  - Small numbers 0-10 (from 85.9% training)")
    print(f"  - Extended to 100 epochs")
    print(f"Target: >60% final accuracy")
    print(f"{'='*60}\n")
    
    print("Creating mixed dataset with small numbers...")
    dataset = create_mixed_small_dataset(n_samples)
    ...
    print("\nSample problems (all with numbers 0-10):")
    ...
    # Create reward function wrapper (exact structure from 54.7% success)
    ...
    print("\nGRPO Configuration:")
    print(f"Mixed dataset + small numbers strategy")
    ...
    print("\nStarting training with combined best strategies...")
    print("Hypothesis: Easier numbers + mixed tasks = >60% accuracy")
    trainer.train()
    ...
    print("Did we break 60%? " + ("YES! 🎉" if overall_accuracy > 0.6 else "Not quite, but close!"))

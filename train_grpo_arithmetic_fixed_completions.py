#!/usr/bin/env python3
"""GRPO arithmetic training with fixed log_completions."""
...
def main():
    ...
    run = wandb.init(
        ...
        name=f"grpo_arithmetic_fixed_completions_b{batch_size}_g{num_generations}",
        config={
            "task": "arithmetic_fixed_completions",
            "n_samples": n_samples,
        ...
        tags=["grpo-setup", "overfit", "arithmetic", "fixed-completions"]
    )
    ...
    # Print dataset info
    print(f"\n{'='*60}")
    print(f"ARITHMETIC GRPO WITH FIXED COMPLETION LOGGING")
    print(f"{'='*60}")
    ...
    print(f"Log Completions: ENABLED (fixed)")
    ...
    # GRPO config with completion logging
    config = GRPOConfig(
        output_dir=f"./grpo_arithmetic_fixed_{seed}",
    ...
    # Train
    print(f"Starting training with fixed completion logging...")
    trainer.train()

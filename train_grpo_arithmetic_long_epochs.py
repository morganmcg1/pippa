#!/usr/bin/env python3
"""GRPO arithmetic training with 100 epochs for higher accuracy."""
...
def main():
    ...
    run = wandb.init(
        ...
        name=f"grpo_arithmetic_long_e{epochs}_b{batch_size}_g{num_generations}",
        config={
            "task": "arithmetic_long_epochs",
            "n_samples": n_samples,
        ...
        tags=["grpo-setup", "overfit", "arithmetic", "long-training"]
    )
    ...
    # Print dataset info
    print(f"\n{'='*60}")
    print(f"ARITHMETIC GRPO WITH LONG TRAINING (100 EPOCHS)")
    print(f"{'='*60}")
    ...
    # GRPO config with long training
    config = GRPOConfig(
        output_dir=f"./grpo_arithmetic_long_{seed}",

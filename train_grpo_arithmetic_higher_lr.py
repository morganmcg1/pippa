#!/usr/bin/env python3
"""GRPO arithmetic training with higher learning rate (1e-5)."""
...
def main():
    # Configuration with higher learning rate
    n_samples = 100
    batch_size = 64
    num_generations = 16
    lr = 1e-5  # 2x higher learning rate
    temperature = 0.7
    epochs = 50
    seed = 654
    ...
    run = wandb.init(
        ...
        name=f"grpo_arithmetic_lr{lr}_b{batch_size}_g{num_generations}",
        config={
            "task": "arithmetic_higher_lr",
            "n_samples": n_samples,
        ...
        tags=["grpo-setup", "overfit", "arithmetic", "higher-lr"]
    )
    ...
    # Print dataset info
    print(f"\n{'='*60}")
    print(f"ARITHMETIC GRPO WITH HIGHER LEARNING RATE")
    print(f"{'='*60}")
    print(f"Dataset size: {len(dataset)} problems")
    print(f"Batch size: {batch_size}")
    print(f"Generations per prompt: {num_generations}")
    print(f"Learning rate: {lr} (2x higher)")
    ...
    # GRPO config with higher learning rate
    config = GRPOConfig(
        output_dir=f"./grpo_arithmetic_lr_{seed}",
        num_train_epochs=epochs,
        per_device_train_batch_size=batch_size,
        gradient_accumulation_steps=1,
        learning_rate=lr,  # Higher learning rate
        num_generations=num_generations,
        temperature=temperature,

#!/usr/bin/env python3
"""
GRPO training with weighted sampling - oversample multiplication and subtraction.
These operations showed lower performance in our best models.
"""
...
def create_weighted_dataset(n_samples=150):
    """
    Create arithmetic dataset with weighted sampling.
    Oversample multiplication and subtraction which performed poorly.
    """
    prompts = []
    answers = []
    
    # Operation weights based on observed performance
    # Addition: 46.7% (good), Subtraction: 29.8% (poor), Multiplication: 34.2% (poor)
    operation_weights = {
        '+': 0.2,  # Less weight - already performs well
        '-': 0.4,  # More weight - needs improvement
        '*': 0.4   # More weight - needs improvement
    }
    ...
    op_counts = {op: prompts.count(f" {op} ") for op in operations}
    print(f"Dataset operation distribution: {op_counts}")
...
def main():
    ...
    wandb.init(
        ...
        tags=["grpo-setup", "standardized-eval", "periodic-eval", "weighted-sampling", "oversample-hard"],
        config={
            "model": model_name,
            "n_samples": n_samples,
            ...
            "number_range": "0-10",
            "sampling": "weighted - oversample multiplication and subtraction"
        }
    )
    
    print("Creating weighted dataset (oversampling multiplication & subtraction)...")
    dataset = create_weighted_dataset(n_samples)
    ...
    # GRPO configuration
    config = GRPOConfig(
        output_dir="./grpo-weighted-sampling",
        run_name="weighted_sampling",
    ...
    print(f"- Oversampling multiplication (40%) and subtraction (40%)")
    print(f"- Undersampling addition (20%) which already performs well")
    print(f"- Addressing weaknesses identified in best models")
    print(f"- 0-10 number range (proven effective)")
    print(f"- Periodic evaluation every {eval_every_n_epochs} epochs")
    ...
    print("\nStarting weighted sampling training...")
    trainer.train()

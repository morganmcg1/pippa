#!/usr/bin/env python3
"""
Ultra-high diversity GRPO training with 64 generations per prompt.
Based on smaller numbers approach that achieved 42-45% standardized eval.
"""
...
def create_smaller_numbers_dataset(n_samples=100):
...
def main():
    ...
    # Training configuration - Ultra diversity
    n_samples = 100  # Smaller dataset to fit 64 generations
    batch_size = 512  # Large batch for 64 generations (512 = 8 prompts * 64 gens)
    ...
    wandb.init(
        ...
        name="ultra_diversity_64gen",
        tags=["grpo-setup", "standardized-eval", "periodic-eval", "ultra-diversity", "64-generations"],
        config={
            "model": model_name,
            "n_samples": n_samples,
    ...
    # GRPO configuration
    config = GRPOConfig(
        output_dir="./grpo-ultra-diversity",
        run_name="ultra_diversity_64gen",
        ...
        warmup_ratio=0.05,  # Shorter warmup for stability
        weight_decay=0.01,
        max_grad_norm=1.0,
        optim="adamw_torch",
        scale_rewards=True,
    ...
    print("\nUltra Diversity Configuration:")
    print(f"- 64 generations per prompt (8x normal)")
    print(f"- Batch size: {batch_size} (8 prompts × 64 generations)")
    print(f"- Maximum reward variance for GRPO advantage calculation")
    print(f"- Based on 0-10 numbers approach (best performing)")
    ...
    print("\nStarting ultra diversity training...")
    trainer.train()
